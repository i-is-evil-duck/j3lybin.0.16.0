...
// Upload ids are 6 alphanumeric characters, generated by the frontend. Only
// allow alphanumerics so they can never be used to escape the data/meta
// directories.
fn isValidId(id: []const u8) bool {
    if (id.len != 6) return false;
    for (id) |c| {
...
// Ids are stored and looked up in lowercase so that links are
// case-insensitive: AbcDef and abcdef resolve to the same upload.
// Caller must ensure buf.len >= id.len (ids are 6 chars).
        ...
        if (!request_error) {
            if (std.mem.eql(u8, req.path, "/")) {
                ...
                if (req.upload_id.len == 0 or req.total_chunks == 0 or !isValidId(req.upload_id)) {
                    ...
                    const norm_id = normalizeId(req.upload_id, &id_buf);
                    const safe_filename = try sanitizeFilename(state.allocator, req.filename);
                    try handleChunkRequest(io, addr, norm_id, req.chunk_idx, req.total_chunks, safe_filename, req.ttl, req.total_size, body, writer, connection);
                ...
                if (!isValidId(id)) {
                    ...
                    var id_buf: [6]u8 = undefined;
                    const norm_id = normalizeId(id, &id_buf);
                    try sendDownload(io, writer, norm_id, connection);
                }
            } else {
                try sendResponse(writer, 404, "text/plain", "Not found", connection);
            }
